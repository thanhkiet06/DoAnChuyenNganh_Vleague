<?php
require '../connect.php';

$keyword = $_GET['q'] ?? '';
$vitri = $_GET['vitri'] ?? '';

$results = [];

if ($keyword !== '') {

    // BẮT BUỘC: thêm % để tìm 1 ký tự
    $keywordLike = '%' . $keyword . '%';

    $sql = "
        SELECT 
            'team' AS type,
            d.ID_DOI_BONG,
            d.TEN_DOI_BONG,
            d.HUAN_LUYEN_VIEN,
            d.LOGO,
            NULL AS HO_TEN,
            NULL AS VI_TRI,
            NULL AS ANH_DAI_DIEN
        FROM DOI_BONG d
        WHERE d.TEN_DOI_BONG LIKE ?
           OR d.HUAN_LUYEN_VIEN LIKE ?

        UNION ALL

        SELECT 
            'player' AS type,
            d.ID_DOI_BONG,
            d.TEN_DOI_BONG,
            d.HUAN_LUYEN_VIEN,
            d.LOGO,
            c.HO_TEN,
            c.VI_TRI,
            c.ANH_DAI_DIEN
        FROM CAU_THU c
        JOIN DOI_BONG d ON c.ID_DOI_BONG = d.ID_DOI_BONG
        WHERE c.HO_TEN LIKE ?
    ";

    if ($vitri !== '') {
        $sql .= " AND c.VI_TRI = ?";
    }

    $stmt = $conn->prepare($sql);

    if ($vitri !== '') {
        $stmt->bind_param("ssss", $keywordLike, $keywordLike, $keywordLike, $vitri);
    } else {
        $stmt->bind_param("sss", $keywordLike, $keywordLike, $keywordLike);
    }

    $stmt->execute();
    $result = $stmt->get_result();

    while ($row = $result->fetch_assoc()) {
        $team_id = $row['ID_DOI_BONG'];

        if (!isset($results[$team_id])) {
            $results[$team_id] = [
                'team_name' => $row['TEN_DOI_BONG'],
                'coach' => $row['HUAN_LUYEN_VIEN'],
                'logo' => $row['LOGO'],
                'players' => []
            ];
        }

        if ($row['type'] === 'player') {
            $results[$team_id]['players'][] = [
                'name' => $row['HO_TEN'],
                'position' => $row['VI_TRI'],
                'avatar' => $row['ANH_DAI_DIEN']
            ];
        }
    }

    $stmt->close();
}
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
    <title>Tìm kiếm đội bóng hoặc cầu thủ</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background:#f5f7fa; font-family: Inter, sans-serif; }
        .title { font-size:40px; font-weight:700; text-align:center; color:#c80e2a; margin-bottom:30px; }
        .result-box { background:#fff; border-radius:10px; padding:20px; box-shadow:0 4px 12px rgba(0,0,0,.05); }
        .item-row { display:flex; align-items:center; margin-bottom:10px; }
        .player-row { margin-left:60px; }
        .logo,.avatar { width:48px;height:48px;border-radius:8px;object-fit:cover;margin-right:10px; }
    </style>
</head>
<body>

<div class="container mt-4">
    <h1 class="title">🔍 Tìm kiếm đội bóng hoặc cầu thủ</h1>

    <form method="GET" class="row g-2 mb-4">
        <div class="col-md-6">
            <input type="text" name="q" class="form-control"
                   value="<?= htmlspecialchars($keyword) ?>"
                   placeholder="Chỉ cần nhập 1 ký tự..." required>
        </div>
        <div class="col-md-3">
            <select name="vitri" class="form-select">
                <option value="">-- Tất cả vị trí --</option>
                <option value="Thủ môn" <?= $vitri=='Thủ môn'?'selected':'' ?>>Thủ môn</option>
                <option value="Hậu vệ" <?= $vitri=='Hậu vệ'?'selected':'' ?>>Hậu vệ</option>
                <option value="Tiền vệ" <?= $vitri=='Tiền vệ'?'selected':'' ?>>Tiền vệ</option>
                <option value="Tiền đạo" <?= $vitri=='Tiền đạo'?'selected':'' ?>>Tiền đạo</option>
            </select>
        </div>
        <div class="col-md-3">
            <button class="btn btn-danger w-100">Tìm kiếm</button>
        </div>
    </form>

<?php if ($keyword !== ''): ?>
<div class="result-box">
    <h5>Kết quả:</h5>

    <?php if (!empty($results)): ?>
        <?php foreach ($results as $team): ?>
            <div class="item-row">
                <?php if ($team['logo']): ?>
                    <img src="<?= str_starts_with($team['logo'],'http')?$team['logo']:'../'.$team['logo'] ?>" class="logo">
                <?php endif; ?>
                <div>
                    <strong><?= htmlspecialchars($team['team_name']) ?></strong><br>
                    HLV: <?= htmlspecialchars($team['coach']) ?>
                </div>
            </div>

            <?php foreach ($team['players'] as $player): ?>
                <div class="item-row player-row">
                    <?php if ($player['avatar']): ?>
                        <img src="<?= str_starts_with($player['avatar'],'http')?$player['avatar']:'../'.$player['avatar'] ?>" class="avatar">
                    <?php endif; ?>
                    <div>
                        <?= htmlspecialchars($player['name']) ?> - <?= htmlspecialchars($player['position']) ?>
                    </div>
                </div>
            <?php endforeach; ?>

        <?php endforeach; ?>
    <?php else: ?>
        <p class="text-muted">Không có kết quả</p>
    <?php endif; ?>
</div>
<?php endif; ?>

</div>
</body>
</html>
